...
function loadLocalization( $languageCode )
{
  $localization = [
    'ru' => [
      ...
      'f027cf381dfe37f0ae92ad6005f557581836eee9' => 'Директория для session.save_path, которая настроена как %s не имеет прав на запись. Невозможно продолжить без правильной работы сессий.',
...
if ( $section == 'install' && !isset( $_POST['action'] ) ) {
  ...
  if ( !is_writable( session_save_path() ) ) {
    $noProceed = true;
    addWarning( sprintf( L( 'The directory of session.save_path that points to %s does not have write permissions. Cannot proceed without working sessions.' ), session_save_path() ), 4, L( 'System check' ) );
  }
